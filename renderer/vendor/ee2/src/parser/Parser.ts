...
function parseModifiers(section: string[], item: ParsedItem) {
  ...
  if (!recognizedLine) {
    // [EE2-VENDOR PATCH] Simple copies (in-game market "copy item", plain Ctrl+C)
    // have explicit sections with no advanced {} headers and no rune/enchant
    // suffixes - upstream skips them entirely because its own hotkey always
    // produces advanced copies. Fall through to parseModifiersPoe2's per-line
    // path, which handles bare lines and per-line (crafted)/(desecrated)/...
    // markers, and reports SECTION_SKIPPED when the section matches no stats.
