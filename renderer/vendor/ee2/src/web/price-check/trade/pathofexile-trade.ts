// [EE2-VENDOR TRIMMED] Pure trade-query construction extracted from
// renderer/src/web/price-check/trade/pathofexile-trade.ts.
//
// KEPT: createTradeRequest() (the trade2 search-body builder) plus every pure
// helper it and the fetch-result display parser use, and all type/enum defs.
//
// REMOVED (they require Vue/Electron, which this subtree excludes):
//   - requestTradeResultList() and requestResults() network calls
//   - imports: `Host` (Electron IPC proxy, @/web/background/IPC),
//     `usePoeninja`/`displayRounding`/`DivCurrency` (Vue price store, Prices.ts),
//     `getCurrencyDetailsId` (trends/getDetailsId.ts), `RateLimiter`, `Cache`,
//     `DateTime` (luxon), and the `TradeResponse`/`getTradeEndpoint`/
//     `adjustRateLimits`/`RATE_LIMIT_RULES`/`preventQueueCreation` symbols from
//     ./common that only those two functions used.
//   - the module-level `const cache = new Cache()`.
// The pure `parseFetchResult` family (buildNameBlock/parseModBlock/...) is retained;
// it is exercised by __testExports and is safe to bundle.
...
// [EE2-VENDOR] Inlined from @/web/background/Prices (Vue store) so the kept
// PricingResult type resolves without importing it. Verbatim shape.
type CoreCurrency = {
...
// [EE2-VENDOR] requestTradeResultList() and requestResults() were removed here
// (they used Host.proxy / RateLimiter / Cache / poe.ninja — Vue+Electron).
